...
export function convertMessages(messages, { modelId, system, tools } = {}) {
  ...
  const buildUserInputMessageContext = (extra = {}) => ({
    ...
    if (sysText) {
      history.push({
        userInputMessage: {
          content: sysText,
          origin: 'AI_EDITOR',
          userInputMessageContext: buildUserInputMessageContext(),
        },
      });
      ...
      if (toolResults.length > 0) {
        ...
        history.push({
          userInputMessage: {
            content: '',
            origin: 'AI_EDITOR',
            userInputMessageContext: buildUserInputMessageContext({ toolResults }),
